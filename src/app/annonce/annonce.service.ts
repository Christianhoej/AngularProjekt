import {Annonce} from '../models/annonce.model';
import { Subject } from 'rxjs';
import {Injectable} from '@angular/core';
import {HttpClient} from '@angular/common/http';
import {ResourceURLService} from '../resourceURL.service';
import {AnnonceDataService} from './annonce-data.service';
import {Kategorier} from '../startside/kategorier/kategorier.model';
import {applySourceSpanToExpressionIfNeeded} from '@angular/compiler/src/output/output_ast';

@Injectable({
  providedIn: 'root'
  })
export class AnnonceService {
  annonceændret = new Subject<Annonce[]>();
  annoncer: Annonce[];
  annonce: Annonce;
  /*private annoncer: Annonce[] = [
    new Annonce(
      'Kaj Bojesen Abe',
      700,
      'https://designme.dk/media/catalog/product/cache/1/image/800x800/17f82f742ffe127f42dca9de82fb58b1/3/9/39256-kay-bojesen-abe-lille-eg-ahorn-3/kay-bojesen-abe-eg-og-ahorn-lille-34.jpg',
      'Meget fin Kaj Bojesen abe, der hurtigt kan blive din! \n Køb den nu!',
      'nips',
      '0',
      'test@test.dk',
      ''
    ),
    new Annonce(
      'Stol',
      300,
      'data:image/jpeg;base64,',
      'Havestol',
      'møbler',
      '1',
      'test@test.dk',
      ''
    ),
    new Annonce(
      'Maleri',
      3000,
      'https://mynewart.dk/477-large_default/ape.jpg',
      'Smukt Maleri',
      'kunst',
      '2',
      'test@test.dk',
      ''
    ),
    new Annonce(
      'Strikket sweater',
      250,
      'http://uniqamessen.dk/upload_dir/shop/IMG_2288.wm.jpg',
      'Hjemmelavet strikket sweater',
      'strik',
      '3',
      'test@test.dk',
      ''
    ),
    new Annonce(
      'Krus',
      2500,
      'http://nemmehjemmesider.dk/aspuploader/savefiles/3495/423561image.jpg',
      'Hjemmelavet strikket sweater',
      'ler',
      '4',
      'test@test.dk',
      ''
    ),

  ];*/

  constructor(private http: HttpClient, private resourceURL: ResourceURLService,
              private annonceDataService: AnnonceDataService)  { }

 /* setAnnoncer(annoncer: Annonce[]) {
    this.annoncer = annoncer;
    this.annonceændret.next(this.annoncer.slice());
  }*/

  /*getAnnoncer() {
    return this.annonceDataService.hentAnnoncer();
  }*/

  getAnnoncer() {
    return this.http.get(this.resourceURL.adsURL);
  }

  getAnnonce(id: string) {
    return this.http.get(this.resourceURL.adsURL + '/' + id);
  }

  filtrerAnnonce(kategori: string) {
    return this.http.get(this.resourceURL.adsCategoryURL + '/' + kategori);
  }

  redigerAnnonce(annonce: Annonce, id: string) {
    return this.http.put(this.resourceURL.adsURL + '/' + id, annonce);
  }

  sletAnnonce(id: string) {
    return this.http.delete(this.resourceURL.adsURL + '/' + id);
  }





/*
  getAnnonce(index: number) {
    return this.annoncer[index];
  }

  addAnnonce(annonce: Annonce) {
    this.annoncer.push(annonce);
    this.annonceændret.next(this.annoncer.slice());
  }*/
}
